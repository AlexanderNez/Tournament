  ...
  list.innerHTML = ids.map((id) => {
    ...
    return `
      <div class="team-row-card" data-id="${id}">
        ...
        <div class="field">
          <label>Player 1 (Epic ID)</label>
          <input type="text" class="t-p1" value="${escapeHtml(t.player1)}" placeholder="Player 1 Epic ID" />
        </div>
        <div class="field">
          <label>Player 2 (Epic ID)</label>
          <input type="text" class="t-p2" value="${escapeHtml(t.player2)}" placeholder="Player 2 Epic ID" />
        </div>
        <div class="field muted">
          <label>Discord 1</label>
          <input type="text" class="t-d1" value="${escapeHtml(t.discord1)}" placeholder="optional" />
        </div>
        <div class="field muted">
          <label>Discord 2</label>
          <input type="text" class="t-d2" value="${escapeHtml(t.discord2)}" placeholder="optional" />
        </div>
      </div>
    `;
  }).join("");
...
function parseRosterPaste(raw, hostNumbers) {
  ...
  lines.forEach((line) => {
    ...
    // First column is usually the team number — drop it if it's just a number,
    // otherwise treat the row as [discord, epic] with no number column.
    let teamNum = null, discordCol, epicCol;
    if (/^\d+$/.test(cols[0])) {
      teamNum = cols[0];
      discordCol = cols[1];
      epicCol = cols[2] || "";
    } else {
      // Header row or a two-column row — skip obvious headers.
      if (/discord/i.test(cols[0]) || /team/i.test(cols[0])) return;
      discordCol = cols[0];
      epicCol = cols[1] || "";
    }

    if (teamNum && hostNumbers.has(teamNum)) return;

    const discordParts = discordCol.split("&").map((s) => s.trim());
    const epicParts = epicCol.split("&").map((s) => s.trim());

    teams.push({
      name: teamNum ? `Team ${teamNum}` : "",
      player1: epicParts[0] || "",
      player2: epicParts[1] || "",
      discord1: discordParts[0] || "",
      discord2: discordParts[1] || ""
    });
  });
...
document.getElementById("parseRosterBtn").addEventListener("click", () => {
  ...
  const hostRaw = document.getElementById("hostNumbers").value;
  const hostNumbers = new Set(hostRaw.split(",").map((s) => s.trim()).filter(Boolean));
  const msg = document.getElementById("parseMsg");

  const parsed = parseRosterPaste(raw, hostNumbers);
  if (!parsed.length) {
    msg.textContent = "Couldn't find any team rows in that paste — check the format and try again.";
    return;
  }

  state.teams = {};
  parsed.slice(0, 20).forEach((team, i) => {
    state.teams[`team${i + 1}`] = team;
  });

  msg.textContent = `Filled ${Math.min(parsed.length, 20)} team slots below${parsed.length > 20 ? " (only the first 20 were used)" : ""}. Review, then click "Save all teams".`;
  renderTeamsPanel();
});
...
document.getElementById("saveTeamsBtn").addEventListener("click", async () => {
  ...
  rows.forEach((row) => {
    ...
    const data = {
      ...
      discord1: row.querySelector(".t-d1").value.trim(),
      discord2: row.querySelector(".t-d2").value.trim()
    };
    state.teams[id] = data;
